...
def generate_dashboard(qpu):
    ...
    for i, (a, b) in enumerate(qpu["edges"]):
        qpu_node_data += f"\n{i},{a},{b}"
    panels.append(dashboards.generate_node_graph_panel(qpu["name"], qpu_node_data,
                                                       gridPos=GridPos(h=defPanelH, w=defPanelW, x=0, y=gridposY)), )
...
def upload_dashboard_to_grafana(json, server, api_key, auth, verify=True):
    '''
    upload_dashboard_to_grafana tries to upload dashboard to grafana and prints response
    ...
    r = requests.post(f"{server}/dashboards/db", data=json, headers=headers, verify=verify, auth=auth)
    print(r.text)
    return

"""
def upload_datasource_to_grafana(server, api_key, auth, verify=True):
    '''
    upload_dashboard_to_grafana tries to upload dashboard to grafana and prints response
    ...
    headers = {'Authorization': f"Bearer {api_key}", 'Content-Type': 'application/json'}
    q = requests.get(f"{server}/datasources", headers=headers, verify=verify, auth=auth)
    print(q.text)
    ...
    r = requests.post(f"{server}/datasources", data=data, headers=headers, verify=verify, auth=auth)
    print(r.text)
    return
"""
import glob
...
def main():
    ...
    for qpu in qpu_config['qpus']:
        ...
        upload_dashboard_to_grafana(dashboard_json, GRAFANA_URL, grafana_key, (ADMIN_USERNAME, ADMIN_PASSWORD))
